...
fn find_max_overlaps(pos_range: Vec<(i64, i64)>) -> Vec<(i64, i64)> {
...
fn find_search_volumes(bots: &[Bot]) -> Vec<Volume> {
    ...
    for (x_lo, x_hi) in &x_overlaps {
        for (y_lo, y_hi) in &y_overlaps {
            for (z_lo, z_hi) in &z_overlaps {
                let vol = Volume {
                    x_lo: *x_lo,
                    x_hi: *x_hi,
                    y_lo: *y_lo,
...
fn best_spot(bots: &[Bot]) -> i64 {
    let volumes = find_search_volumes(bots);
    println!("{} volumes", volumes.len());
    for vol in &volumes {
        println!("{:?}", vol);
        //println!("  need to search {} points", vol.n_points());
    }
    let (n, d) = volumes
        .iter()
        .map(|v| best_in_volume(bots, v))
        .max()
        .expect("No best spot");
